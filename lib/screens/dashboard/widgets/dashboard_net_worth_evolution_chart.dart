...
class _DashboardNetWorthEvolutionChartState extends State<DashboardNetWorthEvolutionChart> {
  ...
  // Fase 2: Método para calcular la proyección al final del mes usando últimos 3 meses
  double _calculateProjectedEndOfMonth(DateTime startOfMonth, DateTime endOfMonth) {
    ...
    // Calcular el crecimiento promedio diario basado en los últimos 3 meses
    final averageDailyGrowth = _calculateAverageDailyGrowthLast3Months();
    
    // Si no hay suficiente historial, usar método simple del mes actual
    if (averageDailyGrowth == null) {
      final startOfMonthNetWorth = _calculateNetWorthAtDate(startOfMonth);
      final dailyGrowth = daysPassed > 0 
          ? (_currentMonthNetWorth - startOfMonthNetWorth) / daysPassed
          : 0.0;
      return _currentMonthNetWorth + (dailyGrowth * daysRemaining);
    }
    
    // Proyectar usando el crecimiento promedio de 3 meses
    return _currentMonthNetWorth + (averageDailyGrowth * daysRemaining);
  }
  
  // Calcula el crecimiento diario promedio de los últimos 3 meses completos
  double? _calculateAverageDailyGrowthLast3Months() {
    final now = DateTime.now();
    final currentMonth = DateTime(now.year, now.month, 1);
    
    List<double> monthlyGrowths = [];
    List<int> daysInMonths = [];
    
    // Analizar los últimos 3 meses completos (excluyendo el mes actual)
    for (int i = 1; i <= 3; i++) {
      final monthStart = DateTime(currentMonth.year, currentMonth.month - i, 1);
      final monthEnd = DateTime(currentMonth.year, currentMonth.month - i + 1, 1)
          .subtract(Duration(days: 1));
      
      // Verificar que el mes esté en nuestro rango de datos
      final earliestDate = DateTime(now.year, now.month - _selectedMonths, 1);
      if (monthStart.isBefore(earliestDate)) {
        continue;
      }
      
      final startValue = _calculateNetWorthAtDate(monthStart);
      final endValue = _calculateNetWorthAtDate(monthEnd);
      final monthlyGrowth = endValue - startValue;
      final daysInMonth = monthEnd.day;
      
      monthlyGrowths.add(monthlyGrowth);
      daysInMonths.add(daysInMonth);
    }
    
    // Si no tenemos suficientes datos, retornar null
    if (monthlyGrowths.isEmpty) {
      return null;
    }
    
    // Calcular el crecimiento diario promedio ponderado
    double totalWeightedGrowth = 0.0;
    int totalDays = 0;
    
    for (int i = 0; i < monthlyGrowths.length; i++) {
      totalWeightedGrowth += monthlyGrowths[i];
      totalDays += daysInMonths[i];
    }
    
    return totalDays > 0 ? totalWeightedGrowth / totalDays : 0.0;
  }

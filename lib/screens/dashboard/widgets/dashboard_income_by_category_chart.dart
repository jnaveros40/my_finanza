// dashboard_income_by_category_chart.dart
// Componente modular para mostrar el gráfico de ingresos por categoría
...
class DashboardIncomeByCategoryChart extends StatelessWidget {
  final List<Movement> movements;
  final List<Category> categories;

  const DashboardIncomeByCategoryChart({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final incomes = movements.where((m) => m.type == 'income');

    // Agrupar ingresos por categoría y sumar los montos
    final incomeByCategory = incomes.groupFoldBy<String, double>(
      (movement) {
    ...
    // Calcular el total de ingresos para el título
    double totalIncome = incomeByCategory.values.fold(0.0, (sum, amount) => sum + amount);

    // Construir las secciones del gráfico de torta
    final sections = _buildPieChartSections(incomeByCategory, totalIncome, context);    // Si no hay ingresos, mostrar un mensaje
    if (totalIncome <= 0) {
      return Card(
        ...
        child: ExpansionTile(
          ...
          title: Text('Ingresos por Categoría', style: Theme.of(context).textTheme.titleLarge),
          subtitle: Column(
          ...
          children: [
            Padding(
              ...
              child: Center(
                child: Text(
                  'No hay datos de ingresos por categoría en este rango de fechas para el gráfico.',
      ...
      child: ExpansionTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            const SizedBox(height: 2),
            Text(
              'Total: ${_formatCurrency(totalIncome, 'COP')}',
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                color: Theme.of(context).textTheme.bodySmall?.color,
                fontWeight: FontWeight.w500,
              ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Resumen de Ingresos', style: Theme.of(context).textTheme.titleMedium),
                    Spacer(),
                    Tooltip(
                      message: 'Distribución de ingresos por categoría en el período seleccionado',
                ...
                Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                      _formatCurrency(totalIncome, 'COP'),
                ...
                SizedBox(height: 16),
                // Leyenda del gráfico
                _buildChartLegend(incomeByCategory, context),
              ],
            ),
          ),
        ],
